...
WebMentionPing.prototype._updateOrDeleteExistingMentions = function (data) {
  ...
  return knex.table('mentions').where('eid', id).select().bind(this).then(function (existingMentions) {
    ...
    var updates = _.map(existingMentions, function (existingMention) {
      var currentMention, interaction, update;
      ...
      if (!currentMention && !this.findLink(existingMention.normalizedUrl)) {
        console.log('Removing mention:', id, existingMention.normalizedUrl);
        update = knex.table('mentions')
          .where('eid', id)
          .whereIn('normalizedUrl', existingMention.normalizedUrl)
          .del();
      } else {
        interaction = entry.type && _.some(entry.data.interactions, function (target) {
        ...
        if (interaction !== existingMention.interaction) {
          ...
          console.log('Updating mention:', id, existingMention.normalizedUrl);
          update = knex.table('mentions')
            .where('eid', id)
