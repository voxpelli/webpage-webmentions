...
WebMentionPing.prototype.parseSourcePage = function (body) {
  var $, base, links, linked, i, length, href, sourceParts;
  ...
  for (i = 0, length = links.length; i < length; i += 1) {
    ...
    try {
      if (href && urlTools.normalizeUrl(href, { relativeTo: this.sourceBase }) === this.normalizedTarget) {
        linked = true;
        break;
      }
    } catch (e) {}
...
WebMentionPing.prototype.saveMention = function (mention) {
  return knex
    .table('entries').pluck('id').where('normalizedUrl', mention.normalizedUrl)
    .bind(this)
    .then(function (id) {
      //TODO: Update the entry content and notify all mentions about it
      return id.length ? id : knex.table('entries').insert(mention, 'id');
    })
    .catch(function (err) {
      // Unique constraint error
      if (err.code === '23505') {
        return knex.table('entries').pluck('id').where('normalizedUrl', mention.normalizedUrl);
      } else {
        throw err;
      }
    })
    .then(function (id) {
      return knex.table('mentions').insert({
        url : this.target,
    ...
    .then(function (id) {
      return Promise.all([
        knex.table('sites').update('lastmention', knex.fn.now()).where('hostname', this.targetHost),
        knex.raw("NOTIFY mentions, '" + JSON.stringify({ eid : id[0], }) + "'").exec(),
      ]);
    })
    .catch(function (err) {
      // Unique constraint error, ignore for now
      if (err.code === '23505') {
        //TODO: Add test for both correct update + duplicate ping behaviour
        //TODO: Update interaction-status of mention
        //TODO: Ensure that a notification is sent for new mentions and/or updated entry data (and maybe for updated mention interaction status)
      } else {
        throw err;
      }
    });
