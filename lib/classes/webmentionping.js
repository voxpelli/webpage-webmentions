...
WebMentionPing = function (source, target) {
  ...
  this.normalizedTarget = normalizeUrl(target);

  this.sourceHost = url.parse(source).hostname;
  this.targetHost = url.parse(target).hostname;
};
...
WebMentionPing.prototype.parseSourcePage = function (body) {
  ...
  for (i = 0, length = links.length; i < length; i += 1) {
    ...
    console.log(href, normalizeUrl(href, this.normalizedSource), this.normalizedTarget); //FIXME: Rmeove this console log
    if (href && normalizeUrl(href, this.normalizedSource) === this.normalizedTarget) {
      linked = true;
      break;
    }
  }
