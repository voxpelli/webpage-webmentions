...
WebMentionPing.prototype.parseSourcePage = function (body) {
  var $, $mf, base, linked, sourceParts;
  ...
  if (!linked) {
    ...
    return Promise.denodeify(microformats.parseDom)($mf, $mf.root(), {
      filters : ['h-entry'],
      logLevel : 0,
...
WebMentionPing.prototype.saveMention = function (mention) {
  ...
  updateMention = function () {
    return knex.table('entries')
      .where('normalizedUrl', mention.normalizedUrl)
      .update({
        updated: mention.fetched,
        type: mention.type,
        data : mention.data,
        raw : mention.raw,
      }, 'id').then(function (id) {
  ...
  return updateMention()
    .bind(this)
    .then(function (id) {
      return id.length ? id : knex.table('entries').insert(mention, 'id');
    })
    .catch(function (err) {
    ...
    .then(function (id) {
      ...
      if (updated) {
        ...
        detectUpdates = knex.table('mentions').where('eid', id[0]).select().bind(this).map(function (existingMention) {
          var interaction, update;

          if (!this.findLink(existingMention.normalizedUrl)) {
            console.log('Removing mention:', existingMention.normalizedUrl);
            remove.push(existingMention.normalizedUrl);
          } else {
            interaction = mention.type && _.some(mention.data.interactions, function (target) {
              return urlTools.normalizeUrl(target, { relativeTo: this.sourceBase }) === existingMention.normalizedTarget;
            }.bind(this)) ? true : false;
            if (interaction !== existingMention.interaction) {
              //TODO: Ensure these updates are broadcast
              update = knex.table('mentions')
                .where('eid', id[0])
                .where('normalizedUrl', existingMention.normalizedTarget)
                .update({
                  updated: mention.fetched,
        ...
        }).then(function (existingMentions) {
          if (
            !_.find(existingMentions, function (existingMention) {
              return existingMention.normalizedUrl === this.normalizedTarget;
            }, this)
          ) {
            newMention = true;
          }
        ...
        newMention = true;
        detectUpdates = Promise.resolve();
      }

      return detectUpdates.then(function () {
        if (newMention) {
          next.push(knex.table('sites').update('lastmention', knex.fn.now()).where('hostname', this.targetHost));
          next.push(
            knex.table('mentions').insert({
              url : this.target,
              normalizedUrl : this.normalizedTarget,
              interaction : mention.type && _.some(mention.data.interactions, function (target) {
                return urlTools.normalizeUrl(target, { relativeTo: this.sourceBase }) === this.normalizedTarget;
              }.bind(this)) ? true : false,
              eid : id[0],
        ...
        if (remove.length) {
          next.push(knex.table('mentions').where('eid', id[0]).whereIn('normalizedUrl', remove).del());
        }

        return Promise.all(next).then(function () {
          return id;
        });
      }.bind(this));
    }).then(function (id) {
      return knex.raw("NOTIFY mentions, '" + JSON.stringify({ eid : id[0], }) + "'").exec();
    });
  };
