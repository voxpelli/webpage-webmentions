...
metaDataFetcher.on('metadata', (entry, context) => {
  if (context.responsesFor) {
    ...
    ((entry.getData().raw.microformats || {}).items || [])
      .map(item => (mfTools.getUValues(item, 'url') || [])[0])
      .filter(url => url && !fetchChain.includes(urlTools.normalizeUrl(url)))
      .forEach(url => {
  ...
  // FIXME: Add a catch() here! + add linting for missing catches!
});
...
module.exports = function (app) {
  ...
  app.post('/api/webmention', (req, res) => {
    ...
    if (
      !source ||
      !target ||
      !urlTools.isHttpUrl.test(source) ||
      !urlTools.isHttpUrl.test(target)
    ) {
      res.status(400).json({
        error: true,
