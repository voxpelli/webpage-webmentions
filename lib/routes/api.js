...
module.exports = function (app) {
  ...
  app.post('/api/webmention', function (req, res, next) {
    ...
    f = ffDb(function (pgClient) {
      ...
      function (result) {
        client.query('INSERT INTO mentions (url, eid, hostname) VALUES ($1, $2, $3)', [target, result.rows[0].id, targetHost], f.wait());
        client.query('UPDATE sites SET lastmention = NOW() WHERE hostname = $1', [targetHost], f.wait());
      }
    ).onSuccess(function () {
      res.json({
        success : true
