...
distillTargets = function (mention, target) {
  var isTarget = function (checkTarget) {
    var result = false;
    ...
    if (!result && !_.isEmpty(target.site)) {
      result = [].concat(target.site).forEach(function (targetSite) {
        return checkSite === targetSite;
      });
    }
    if (!result && !_.isEmpty(target.path)) {
      //TODO: Enable distilling for this as well!
    }
