...
getMentions = function (target) {
  if (_.isPlainObject(target) || !target) {
    return Promise.resolve((target && target.example !== undefined) ? require('../utils/sample-data').mentions() : []);
  }

  return knex('entries')
    .select('entries.url', 'entries.data')
    .innerJoin('mentions', 'entries.id', 'mentions.eid')
    .where('mentions.normalizedUrl', urlTools.normalizeUrl(target))
    .map(function (row) {
...
module.exports = function (app) {
  app.get('/api/embed', function (req, res) {
    getMentions(req.query.example !== undefined ? { example: true } : req.query.url).then(function (mentions) {
