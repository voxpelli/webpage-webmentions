...
addSite = function (res, aid, hostname) {
  var f = ffDb(function (client) {
    client.query('INSERT INTO sites (aid, hostname, created) VALUES ($1, $2, now())', [aid, hostname], f.slot());
  })
  .onSuccess(function (result) {
    res.redirect('/');
  })
...
module.exports = function (app) {
  ...
  app.get('/user/sites', ensureAuthenticated, function (req, res) {
    var f = ffDb(function (client) {
      client.query('SELECT s.hostname, COUNT(eid) AS mentions FROM sites s LEFT JOIN mentions m ON m.hostname = s.hostname WHERE s.aid = $1 GROUP BY s.hostname', [req.passportUser.id], f.slot());
    })
    .onSuccess(function (result) {
      var sites = [];
      result.rows.forEach(function (row) {
